...
let
  opencascade = if qtVersion == 5 then opencascade-occt_7_6 else opencascade-occt;
  inherit (if qtVersion == 5 then libsForQt5 else kdePackages)
    qtbase
    qttools
    qtwebengine
    ;

  inherit (if qtVersion == 5 then libsForQt5.qt5 else kdePackages)
    qtwayland
    ;
...
freecad-utils.makeCustomizable (
  stdenv.mkDerivation (finalAttrs: {
    ...
    src = fetchFromGitHub {
      ...
      inherit (finalAttrs)
        hash
        ; # using finalAttrs for version only has meaning if we can also override the hash
    ...
    nativeBuildInputs =
      ...
      ++ lib.optionals (qtVersion == 6) [ kdePackages.wrapQtAppsHook ];
